...
tree.model<- jags.model (file = textConnection(NormalMean),
                        data = data,
                        n.chains = 5)
tree.out   <- coda.samples (model = tree.model,
                            variable.names = c("mu"),
                            n.iter = 1000)



cumuplot(tree.out,probs=c(0.025,0.25,0.5,0.75,0.975))
##CI interval is not clean near the ends, so more iterations are needed


tree.out2   <- coda.samples (model = tree.model,
                            variable.names = c("mu"),
                            n.iter = 10000)



cumuplot(tree.out2,probs=c(0.025,0.25,0.5,0.75,0.975))

##the tails are much cleaner now, so the data is better

gelman.diag(tree.out2)

GBR <- gelman.plot(tree.out2)

burnin <- GBR$last.iter[tail(which(GBR$shrink[,,2] > 1.1),1)+1]
if(length(burnin) == 0) burnin = 1
tree.burn <- window(tree.out2,start=burnin)

plot(tree.burn)
effectiveSize(tree.burn)

##my effective size is well above 5,000, which is suggested
summary(tree.burn)
...
mu ~ dnorm(mu0,T) # prior on the mean 
sigma~dunif(1,10)
S =1/sigma^2 #prior on the precision
for(i in 1:N){
X[i] ~ dnorm(mu,S) # data model
}
}
...
##my effective size is way above 5,000, which is suggested
summary(tree.burn)

```
I used a distribution for S that had to be positive, since none of the values would make sense if they were allowed to be negative. 
There is no noticeable difference in the distribution of mu, which makes sense, as mu and S are not dependent on each other.
