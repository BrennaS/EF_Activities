...
abd<-tapply(fia$TREECOUNT,fia$SPCD,sum, na.rm=TRUE)
s_abd<-sort(abd,decreasing=TRUE)
log_s<-log(s_abd)
plot(log_s, xlab="Rank", ylab="logAbundance")
...
```{r}
flux<-getURL("http://fluxnet.ornl.gov/site_status")
flux_table = readHTMLTable(flux)
flux_read<-flux_table[]$status_table
newyear<-as.numeric(levels(flux_read[,27]))[flux_read[,27]]
place<-substring(flux_read[,2],1,2)
flux_country<-tapply(newyear,place,sum, na.rm=TRUE)
View(flux_country)
```
...
```{r}
grep('^#',myCode)
#a line(323) has a space before the #, so to include that it would be:
grep('^ ? ? ?#',myCode)
```
...
```
#Warning message:
#running command 'wget #http://flux.aos.wisc.edu/data/cheas/wlef/netcdf/US-PFa-WLEF-TallTowerClean-2012-L0-vF#eb2013.nc' had status 127 
...
```{r}
plot(time,EVI,xlab="Time")
#do not have CO2 flux observations
```
cron
----
...
```
##psuedocode##
#pull data originally
  find data in system
  download data to system
  save path for cron table to use
#set cron table
  determine path and final destination
  create path
  set table
#updating
 find the max data for each data entry
  #assuming update time is supplied
 update data for all data entrys with a max date between my last update and theirs

#cron
MAILTO=brennas@bu.edu
* 23 * * sun /home/'file_name path'/'program'


crontab( e, HOME="Brenna'sEF_Activities",
         MAILTO=brennas@bu.edu
         * 23 * * sun /home/'file_name path'/'program'
         )
```
