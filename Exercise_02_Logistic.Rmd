...
#For the growth rate of r=1.95, the rise is slightly steeper than the standard logistic growth until it reaches the carrying capacity, when the data begins to oscillate around the carrying capacity. As the growth rate rises, the oscillations get larger and less even, and the inital growth curve gets steeper as well. 
...
```{r}
x<-seq(1,10, by=1)
plot(x,dbinom(x,15,0.4),type='l') 
q<-seq(1,10,by=1)
plot(q,pbinom(q,15,0.4),type='l')
plot(p,qbinom(p,15,0.4),type='l') 
for(i in 1:4){hist(rbinom(n[i],15,0.4),main=n[i],probability=TRUE,breaks=40) 
lines(x,dbinom(x,15,0.4),col=2)}
...
```{r}
x=rlnorm(x,meanlog=0,sdlog=1)
y=sin(x)
hist(x,main="Original distribution",breaks=40)
abline(v=quantile(x,c(0.025,0.5,0.975)),lty=c(2,1,2),col=2)
abline(v=mean(x),col=3,lwd=3,lty=3)
legend('topright', legend=c("Median:1.65 ", "Mean:0.997","95%CI:0.142-7.197"),
   col=c(3,2,2), lty=2:1, cex=0.8)

hist(y,main="Transformed distribution",breaks=40)
abline(v=quantile(y,c(0.025,0.5,0.975)),lty=c(2,1,2),col=2)
abline(v=mean(y),col=3,lwd=3,lty=3)
legend('topright', legend=c("Median:0.472 ", "Mean:0.558","95%CI:-0.945-0.999"),
   col=c(3,2,2), lty=2:1, cex=0.8)
```
...
```{r}
matplot(n[1:30,1:10], type='l', pch=1, xlab='Time', ylab='Population')
```
...
plot(med,type='l',ylim = c(0,12))
ylo<-apply(n,2,quantile, c(0.025))
yhi<-apply(n,2,quantile, c(0.975))
ciEnvelope(1:30,ylo,yhi,col="lightgrey")
par(new=TRUE)
plot(med,type='l',ylim = c(0,12))
```
...
yhi<-apply(n,2,quantile, c(0.975))
ciEnvelope(1:30,ylo,yhi,col="lightgrey")
ylo2<-apply(n,2,quantile,c(.25))
yhi2<-apply(n,2,quantile,c(0.75))
ciEnvelope(1:30,ylo2,yhi2,col="lightblue")
par(new=TRUE)
plot(med,type='l',ylim = c(0,12))
```
...
```
```{r}
log_curve<-function(r,ylo,yhi,col="lightgrey"){
 n = matrix(n0s,NE,NT)
 for(i in 1:NE){
   for(t in 2:NT){
      n[i,t] = n[i,t-1] + r*n[i,t-1]*(1-n[i,t-1]/K)
